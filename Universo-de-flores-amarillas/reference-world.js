import * as THREE from 'three';
  ...
  let overviewYaw = 0;
  let overviewPolar = Math.atan2(105, 84);
  let overviewDistance = Math.hypot(84, 105);
  let renderEnabled = false;
  let initializationFailed = false;
  let lastRenderTime = 0;
  const lowPower = window.matchMedia('(pointer: coarse)').matches || window.innerWidth < 900 || (navigator.hardwareConcurrency || 8) <= 4;
  const letterGroup = new THREE.Group();
  const letterRaycaster = new THREE.Raycaster();
  let roamActive = false;
  const roamPosition = new THREE.Vector3();
  const lampLights = [];
  ...
  function terrainHeight(x, z) {
    ...
    const pondBasin = Math.exp(-(((x - 19) / 9) ** 2 + ((z + 13) / 7) ** 2) * 2.2) * 1.15;
    return broad - roadFlatten - pondBasin;
  }
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          base += vec3(0.035, 0.11, 0.045) * grid * distFade * 0.25;

          base += vec3(0.015, 0.025, 0.012);
  ...
  const ChromaticInterferenceShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        finalColor *= (1.0 + neonPower * 0.045);
        finalColor = mix(finalColor, finalColor / (finalColor + vec3(0.21)), 0.38);
  ...
  function gardenPathX(z) { return -3 + Math.sin(z * .085 + .5) * 4 - Math.max(0, -z - 5) * .22; }

  // Sunflowers and mountains are built here from geometry for this project.
  // They occupy the positions where the supplied village had trees and mills.
  function buildFlowerGarden() {
    const random = makeRandom(21809);
    const mobile = window.innerWidth < 650;
    const count = mobile ? 185 : 350;
    const petalsPerFlower = 12;
    const stemMat = new THREE.MeshStandardMaterial({ color: 0x286b32, roughness: .83, side: THREE.DoubleSide });
    const leafMat = new THREE.MeshStandardMaterial({ color: 0x41994a, emissive: 0x103717, emissiveIntensity: .28, side: THREE.DoubleSide });
    const petalMat = new THREE.MeshStandardMaterial({ color: 0xffc928, emissive: 0x9c5700, emissiveIntensity: .62, roughness: .5, side: THREE.DoubleSide });
    const heartMat = new THREE.MeshStandardMaterial({ color: 0x563012, emissive: 0x211002, emissiveIntensity: .18, roughness: .94 });
    const stems = new THREE.InstancedMesh(new THREE.CylinderGeometry(.10, .15, 1, 6), stemMat, count);
    const leaves = new THREE.InstancedMesh(new THREE.SphereGeometry(1, 6, 4), leafMat, count * 2);
    const petals = new THREE.InstancedMesh(new THREE.SphereGeometry(1, 7, 5), petalMat, count * petalsPerFlower);
    const hearts = new THREE.InstancedMesh(new THREE.SphereGeometry(1, 12, 7), heartMat, count);
    const dummy = new THREE.Object3D();
    const gold = new THREE.Color();
    const clusters = [[-17,15],[0,12],[11,29],[-12,-2],[3,-19],[29,4],[-29,-30],[3,-38],[32,-33]];
    for (let i = 0; i < count;) {
      let x, z;
      if (random() < .78) {
        const [cx, cz] = clusters[Math.floor(random() * clusters.length)];
        x = cx + (random() + random() + random() - 1.5) * 16;
        z = cz + (random() + random() + random() - 1.5) * 17;
      } else { x = (random() - .5) * 104; z = (random() - .5) * 104; }
      if (Math.abs(x) > 53 || Math.abs(z) > 53) continue;
      if (((x - 19) / 11) ** 2 + ((z + 13) / 9) ** 2 < 1.5) continue;
      if (Math.hypot(x + 28, z + 6) < 10.5) continue;
      if (Math.abs(x - gardenPathX(z)) < 2.15 && z > -31 && z < 48) continue;
      const ground = terrainHeight(x, z);
      const height = 1.45 + Math.pow(random(), .72) * 4.9;
      const radius = .42 + random() * .82;
      const tilt = (random() - .5) * .22;
      const faceYaw = .18 + (random() - .5) * .8;
      dummy.position.set(x, ground + height * .5, z);
      dummy.rotation.set(0, 0, tilt);
      dummy.scale.set(1, height, 1); dummy.updateMatrix(); stems.setMatrixAt(i, dummy.matrix);
      for (let j = 0; j < 2; j++) {
        const side = j ? 1 : -1;
        dummy.position.set(x + side * .52, ground + height * (.35 + j * .15), z + .07);
        dummy.rotation.set(0, 0, side * .42);
        dummy.scale.set(.72, .18, .25); dummy.updateMatrix(); leaves.setMatrixAt(i * 2 + j, dummy.matrix);
      }
      for (let j = 0; j < petalsPerFlower; j++) {
        const angle = j * Math.PI * 2 / petalsPerFlower;
        dummy.position.set(x + Math.cos(angle) * radius * .72 * Math.cos(faceYaw), ground + height + Math.sin(angle) * radius * .72, z + .17 - Math.cos(angle) * radius * .72 * Math.sin(faceYaw));
        dummy.rotation.set(0, faceYaw, angle - Math.PI / 2);
        dummy.scale.set(radius * .27, radius * .66, .09); dummy.updateMatrix();
        const index = i * petalsPerFlower + j;
        petals.setMatrixAt(index, dummy.matrix);
        gold.setHSL(.105 + random() * .035, .95, .49 + random() * .12); petals.setColorAt(index, gold);
      }
      dummy.position.set(x, ground + height, z + .29);
      dummy.rotation.set(0, faceYaw, 0);
      dummy.scale.set(radius * .43, radius * .43, .18); dummy.updateMatrix(); hearts.setMatrixAt(i, dummy.matrix);
      i++;
    }
    for (const mesh of [stems, leaves, petals, hearts]) { mesh.instanceMatrix.needsUpdate = true; villageGroup.add(mesh); }
    const grassCount = mobile ? 1200 : 2800;
    const grass = new THREE.InstancedMesh(new THREE.ConeGeometry(.20, .64, 3), leafMat, grassCount);
    for (let i = 0; i < grassCount;) {
      const x = (random() - .5) * 116, z = (random() - .5) * 116;
      if (((x - 19) / 10) ** 2 + ((z + 13) / 8) ** 2 < 1.12 || Math.hypot(x + 28, z + 6) < 5.5) continue;
      const h = .45 + random() * .8;
      dummy.position.set(x, terrainHeight(x, z) + h * .32, z);
      dummy.rotation.set((random() - .5) * .22, random() * 6.28, (random() - .5) * .3);
      dummy.scale.set(.75 + random() * .5, h, .75 + random() * .5);
      dummy.updateMatrix(); grass.setMatrixAt(i, dummy.matrix);
      grass.setColorAt(i, new THREE.Color().setHSL(.29 + random() * .085, .42 + random() * .25, .28 + random() * .16));
      i++;
    }
    grass.instanceMatrix.needsUpdate = true; villageGroup.add(grass);
    const mountainMat = new THREE.MeshStandardMaterial({ color: 0x315f3b, emissive: 0x102b18, emissiveIntensity: .2, roughness: 1, flatShading: true, side: THREE.DoubleSide });
    const farMountainMat = new THREE.MeshStandardMaterial({ color: 0x244b38, roughness: 1, flatShading: true, side: THREE.DoubleSide });
    const snowMat = new THREE.MeshStandardMaterial({ color: 0x83a56a, emissive: 0x18391e, emissiveIntensity: .14, roughness: .95, flatShading: true });
    // The mountain ring stays beyond the field, leaving its center open.
    const mountains = [[-65,-48,39,28],[-66,-12,30,21],[-64,26,37,25],[-49,-65,43,31],[-25,-70,31,23],[0,-72,45,33],[27,-70,38,27],[53,-60,43,32],[66,-27,35,26],[65,10,30,23],[62,41,27,20]];
    function mountainGeometry(width, height, sides) {
      const positions = [], indices = [];
      const leanX = (random() - .5) * width * .19, leanZ = (random() - .5) * width * .19;
      for (let layer = 0; layer < 3; layer++) {
        const y = [0, height * .38, height * .72][layer];
        const size = [1,.63,.29][layer];
        for (let i = 0; i < sides; i++) {
          const a = i / sides * Math.PI * 2;
          const jag = .84 + random() * .32;
          positions.push(Math.cos(a) * width * .5 * size * jag + leanX * layer / 3, y, Math.sin(a) * width * .5 * size * jag + leanZ * layer / 3);
        }
      }
      positions.push(leanX, height, leanZ);
      for (let layer = 0; layer < 2; layer++) for (let i = 0; i < sides; i++) {
        const a = layer*sides+i, b = layer*sides+(i+1)%sides, c = (layer+1)*sides+i, d=(layer+1)*sides+(i+1)%sides;
        indices.push(a,b,c,b,d,c);
      }
      for (let i = 0; i < sides; i++) indices.push(2*sides+i,2*sides+(i+1)%sides,3*sides);
    ...
    for (const [x, z, width, height] of mountains) {
      const sides = 5 + Math.floor(random() * 4);
      const mountain = new THREE.Mesh(mountainGeometry(width, height, sides), z < -60 ? farMountainMat : mountainMat);
      mountain.position.set(x, -2, z);
      mountain.scale.z = .77 + random() * .58;
      mountain.rotation.y = random() * 6.28;
      villageGroup.add(mountain);
      if (height > 31 && z < -60) {
        const snow = new THREE.Mesh(new THREE.ConeGeometry(width * .115, height * .16, sides), snowMat);
        snow.position.set(x, height * .92 - 2, z); snow.scale.z = mountain.scale.z; snow.rotation.y = mountain.rotation.y;
        villageGroup.add(snow);
      }
    }
  }

  function buildGardenDetails() {
    const random = makeRandom(2109);
    const wood = new THREE.MeshStandardMaterial({ color: 0x785038, roughness: .9 });
    const paleWood = new THREE.MeshStandardMaterial({ color: 0xad7950, roughness: .86 });
    const pathMat = new THREE.MeshStandardMaterial({ color: 0x75573c, roughness: 1, side: THREE.DoubleSide });
    const stoneMat = new THREE.MeshStandardMaterial({ color: 0x747269, roughness: 1, flatShading: true });
    const warmYellow = new THREE.MeshStandardMaterial({ color: 0xf5cf5c, emissive: 0x9e5b12, emissiveIntensity: .45 });
    const foliage = new THREE.MeshStandardMaterial({ color: 0x366d3c, roughness: 1 });
    const add = (geo, mat, x, y, z) => { const mesh = new THREE.Mesh(geo, mat); mesh.position.set(x, y, z); villageGroup.add(mesh); return mesh; };

    // Curved soil tracks lead into the flower clusters and branch toward the water.
    function trail(points, width) {
      const vertices = [];
      for (let i = 0; i < points.length - 1; i++) {
        const [ax, az] = points[i], [bx, bz] = points[i + 1];
        const dx = bx - ax, dz = bz - az, length = Math.hypot(dx, dz);
        const nx = -dz / length, nz = dx / length;
        const w0 = width * (.94 + Math.sin(i * 1.7) * .06), w1 = width * (.94 + Math.sin((i + 1) * 1.7) * .06);
        const corners = [[ax + nx * w0, az + nz * w0],[ax - nx * w0, az - nz * w0],[bx + nx * w1, bz + nz * w1],[bx - nx * w1, bz - nz * w1]];
        const p = corners.map(([x,z]) => [x, terrainHeight(x,z) + .12, z]);
        vertices.push(...p[0],...p[1],...p[2], ...p[1],...p[3],...p[2]);
      }
      const geo = new THREE.BufferGeometry();
      geo.setAttribute('position', new THREE.Float32BufferAttribute(vertices, 3)); geo.computeVertexNormals();
      villageGroup.add(new THREE.Mesh(geo, pathMat));
    }
    trail(Array.from({length: 49}, (_, i) => { const z = 47 - i * 1.58; return [gardenPathX(z), z]; }), 1.2);
    trail([[-8,-12],[-14,-10],[-19,-8],[-24,-6]], .9);
    trail([[gardenPathX(8),8],[1,5],[6,1],[10,-3],[13,-6]], .82);

    // The shallow pond has an irregular bank and soft amber reflections.
    const pondX = 19, pondZ = -13, waterY = terrainHeight(pondX, pondZ) + 1.55;
    const shape = new THREE.Shape();
    for (let i = 0; i <= 48; i++) {
      const angle = i / 48 * Math.PI * 2;
      const ripple = 1 + Math.sin(angle * 5 + .4) * .055 + Math.cos(angle * 9) * .035;
      const x = Math.cos(angle) * 8.7 * ripple, y = Math.sin(angle) * 6.4 * ripple;
      if (i === 0) shape.moveTo(x, y); else shape.lineTo(x, y);
    }
    const bankGeo = new THREE.ShapeGeometry(shape, 48); bankGeo.rotateX(-Math.PI / 2);
    const bank = add(bankGeo, new THREE.MeshStandardMaterial({ color: 0x786d4d, roughness: 1, side: THREE.DoubleSide }), pondX, waterY - .08, pondZ);
    bank.scale.set(1.11, 1, 1.12);
    const water = add(bankGeo.clone(), new THREE.MeshBasicMaterial({ color: 0x287e9c, transparent: true, opacity: .92, side: THREE.DoubleSide }), pondX, waterY, pondZ);
    const glimmer = new THREE.MeshBasicMaterial({ color: 0xffe2a0, transparent: true, opacity: .6, side: THREE.DoubleSide, depthWrite: false });
    for (let i = 0; i < 13; i++) {
      const x = pondX + (random() - .5) * 11, z = pondZ + (random() - .5) * 7;
      const streak = add(new THREE.PlaneGeometry(.3 + random() * 1.6, .06), glimmer, x, waterY + .035, z);
      streak.rotation.x = -Math.PI / 2; streak.rotation.z = .2;
    }

    // A short timber bridge crosses the near edge of the pond.
    const bridgeY = waterY + .85;
    for (let i = 0; i < 14; i++) {
      const x = 10.2 + i * .83;
      const plank = add(new THREE.BoxGeometry(.76, .23, 3.8), i % 3 ? paleWood : wood, x, bridgeY + Math.sin(i / 13 * Math.PI) * .23, -8.5);
      plank.rotation.y = Math.sin(i * .9) * .012;
    }
    for (const z of [-10.5,-6.5]) {
      const rail = add(new THREE.BoxGeometry(12.2,.13,.14), paleWood, 15.7, bridgeY + 1.25, z); rail.rotation.z = -.012;
      for (const x of [10.3,13,15.7,18.4,21.1]) add(new THREE.BoxGeometry(.16,1.35,.16), wood, x, bridgeY + .58, z);
    }

    // A small flower cottage sits behind the path on the opposite side.
    const house = new THREE.Group(); house.position.set(-28, terrainHeight(-28,-6), -6); house.scale.setScalar(1.26); villageGroup.add(house);
    const wall = new THREE.MeshStandardMaterial({ color: 0xc9ad80, roughness: .92 });
    const roofMat = new THREE.MeshStandardMaterial({ color: 0x6c3c37, roughness: .88, flatShading: true });
    const windowMat = new THREE.MeshBasicMaterial({ color: 0xffd879 });
    const box = (w,h,d,mat,x,y,z) => { const mesh = new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat); mesh.position.set(x,y,z); house.add(mesh); return mesh; };
    box(6.2,3.8,5.4,wall,0,2.05,0);
    const roof = new THREE.Mesh(new THREE.ConeGeometry(5.15,2.8,4),roofMat);
    roof.rotation.y = Math.PI / 4; roof.position.y = 5.25; roof.scale.z = .91; house.add(roof);
    box(1.35,2.35,.16,wood,0,1.3,2.78);
    box(1.15,1.05,.10,windowMat,-2.02,2.65,2.79); box(1.15,1.05,.10,windowMat,2.02,2.65,2.79);
    for (const x of [-2.02,2.02]) { box(1.37,.11,.19,paleWood,x,2.65,2.88); box(.11,1.23,.19,paleWood,x,2.65,2.88); box(1.7,.23,.75,wood,x,1.91,3.12); }
    box(.8,1.8,.8,wall,1.82,6.07,-1.25);
    const porch = box(7,.25,2,wood,0,.18,3.39);
    for (const x of [-2.9,2.9]) { box(.18,2.7,.18,paleWood,x,1.6,4.1); box(.75,.78,.75,wood,x,.66,4.4); }
    box(6.3,.16,.18,warmYellow,0,3.9,2.86);
    const houseLight = new THREE.PointLight(0xffb953, 2.1, 17, 2); houseLight.position.set(0,3.4,4.2); house.add(houseLight);

    // Varied shrubs, stones and warm lights keep the clearings from looking empty.
    for (let i = 0; i < 95; i++) {
      const x = (random() - .5) * 108, z = (random() - .5) * 108;
      if (((x - pondX) / 10) ** 2 + ((z - pondZ) / 8) ** 2 < 1.1) continue;
      const size = .28 + random() * 1.05;
      const mesh = add(new THREE.DodecahedronGeometry(size, 0), i % 3 ? foliage : stoneMat, x, terrainHeight(x,z) + size * .35, z);
      mesh.scale.set(1 + random() * .6, .45 + random() * .45, .8 + random() * .5);
      mesh.rotation.y = random() * 6.28;
    }
    for (const [x,z] of [[-11,-15],[-7,7],[8,9],[25,4],[11,-24],[-28,-10]]) {
      const light = new THREE.PointLight(0xffcc70, .82, 12, 2);
      light.position.set(x, terrainHeight(x,z) + 2.3, z); villageGroup.add(light);
      add(new THREE.SphereGeometry(.12,7,5), new THREE.MeshBasicMaterial({ color: 0xffe5a1 }), x, terrainHeight(x,z) + 2.3, z);
    }
    const sparks = new Float32Array(165 * 3);
    for (let i = 0; i < 165; i++) { const x = (random() - .5) * 82, z = (random() - .5) * 82; sparks[i*3] = x; sparks[i*3+1] = terrainHeight(x,z) + 1.3 + random() * 6.7; sparks[i*3+2] = z; }
    const sparkGeo = new THREE.BufferGeometry(); sparkGeo.setAttribute('position', new THREE.BufferAttribute(sparks,3));
    villageGroup.add(new THREE.Points(sparkGeo, new THREE.PointsMaterial({ color: 0xffd779, size: .16, transparent: true, opacity: .67, blending: THREE.AdditiveBlending, depthWrite: false })));
    const petals = new THREE.Group();
    const floatingPetalMat = new THREE.MeshBasicMaterial({ color: 0xffd450, transparent: true, opacity: .66, side: THREE.DoubleSide });
    for (let i = 0; i < 35; i++) {
      const mesh = new THREE.Mesh(new THREE.PlaneGeometry(.22 + random() * .26,.48 + random() * .3), floatingPetalMat);
      mesh.position.set((random()-.5)*45, 3 + random()*12, (random()-.5)*44);
      mesh.rotation.set(random()*6.28,random()*6.28,random()*6.28); petals.add(mesh);
    }
    villageGroup.add(petals);
    dynamicObjects.gardenPetals = petals;
  }

  function buildFlyingHeart() {
    const outline = new THREE.Shape();
    outline.moveTo(0, -.95);
    outline.bezierCurveTo(-.3, -.66, -1.12, .08, -1.12, .62);
    outline.bezierCurveTo(-1.12, 1.24, -.34, 1.28, 0, .68);
    outline.bezierCurveTo(.34, 1.28, 1.12, 1.24, 1.12, .62);
    outline.bezierCurveTo(1.12, .08, .3, -.66, 0, -.95);
    const group = new THREE.Group();
    const geometry = new THREE.ExtrudeGeometry(outline, { depth: .24, bevelEnabled: true, bevelSegments: 2, steps: 1, bevelSize: .12, bevelThickness: .1, curveSegments: 14 });
    const heart = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ color: 0xff7891, emissive: 0x9b284f, emissiveIntensity: .65, metalness: .25, roughness: .3 }));
    group.add(heart);
    const rim = new THREE.LineSegments(new THREE.EdgesGeometry(geometry, 25), new THREE.LineBasicMaterial({ color: 0xffd695, transparent: true, opacity: .8 }));
    group.add(rim);
    const glow = new THREE.Sprite(new THREE.SpriteMaterial({ map: createGlowTexture(128, 'fire'), color: 0xff8ca1, transparent: true, opacity: .18, blending: THREE.AdditiveBlending, depthWrite: false }));
    glow.position.z = -.28; glow.scale.set(5.2, 5.2, 1); group.add(glow);
    const light = new THREE.PointLight(0xff87a0, 1.1, 12, 2); light.position.z = .6; group.add(light);
    group.scale.setScalar(2.05);
    scene.add(group);
    dynamicObjects.flyingHeart = group;
  }

  function buildHiddenLetters() {
    const notes = [
      { x: gardenPathX(29) + 2.7, z: 29, text: 'Te quiero mucho, Naho. Esta pequeña carta es para recordártelo.' },
      { x: -10, z: 18, text: 'Me acuerdo de ti todos los días, incluso en los momentos más sencillos.' },
      { x: 8, z: 4, text: 'Me preocupa cómo estás porque me importas. Espero que hoy tengas un día tranquilo y bonito.' },
      { x: -8, z: -11, text: 'Si alguna vez necesitas de mí, solo llámame. Te escucharé con cariño.' },
      { x: 13, z: -7, text: 'Cuando te nazca hablar, escríbeme. Estaré aquí, sin presionarte.' },
      { x: -25, z: -7, text: 'Aunque estemos lejos, sigues teniendo un lugar muy especial en mi corazón.' }
    ];
    const paper = new THREE.MeshBasicMaterial({ color: 0xffe5a5, side: THREE.DoubleSide });
    const flap = new THREE.MeshBasicMaterial({ color: 0xf6c76c, side: THREE.DoubleSide });
    const seal = new THREE.MeshBasicMaterial({ color: 0xf68697 });
    const hitMaterial = new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false });
    const flapShape = new THREE.Shape(); flapShape.moveTo(-.83,.48); flapShape.lineTo(0,-.16); flapShape.lineTo(.83,.48); flapShape.closePath();
    notes.forEach((note, index) => {
      const marker = new THREE.Group();
      const y = terrainHeight(note.x, note.z) + 2.55;
      marker.position.set(note.x, y, note.z);
      marker.userData.baseY = y;
      marker.userData.letterIndex = index;
      const envelope = new THREE.Mesh(new THREE.PlaneGeometry(1.68,1.12), paper); marker.add(envelope);
      const triangle = new THREE.Mesh(new THREE.ShapeGeometry(flapShape), flap); triangle.position.z = .025; marker.add(triangle);
      const heartSeal = new THREE.Mesh(new THREE.SphereGeometry(.14,8,6), seal); heartSeal.position.set(0,-.13,.09); heartSeal.scale.z = .25; marker.add(heartSeal);
      const halo = new THREE.Sprite(new THREE.SpriteMaterial({ map:createGlowTexture(128,'fire'),color:0xffd98a,transparent:true,opacity:.32,blending:THREE.AdditiveBlending,depthWrite:false }));
      halo.position.z = -.2; halo.scale.set(3.2,2.6,1); marker.add(halo);
      const target = new THREE.Mesh(new THREE.BoxGeometry(2.8,2.4,.3),hitMaterial); target.position.z = .15; marker.add(target);
      marker.traverse(child => { child.userData.letterIndex = index; });
      letterGroup.add(marker); letterTargets.push(marker); hiddenLetters.push(note);
    });
  ...
  function updateOverviewCamera() {
    if (!camera) return;
    const horizontal = Math.sin(overviewPolar) * overviewDistance;
    camera.position.set(
      Math.sin(overviewYaw) * horizontal,
      2 + Math.cos(overviewPolar) * overviewDistance,
      Math.cos(overviewYaw) * horizontal
    );
  ...
  function init() {
    ...
    document.getElementById('reference-world').appendChild(renderer.domElement);
  ...
  function setupUI() {
    ...
    toggle.addEventListener('click', () => {
      ...
      toggle.innerText = panel.classList.contains('minimized') ? '+' : 'âˆ’';
    });
  ...
  function buildMaterials() {
    terrainMaterial = makeNeonMeshMaterial('#12351d', '#1c4a2a');
  ...
  function buildLighting() {
    const hemi = new THREE.HemisphereLight(0xa998b3, 0x16351e, 1.12);
    scene.add(hemi);

    const moonLight = new THREE.DirectionalLight(0xffd8a0, 2.25);
    moonLight.position.copy(moonDirection).multiplyScalar(60);
    scene.add(moonLight);

    const underLight = new THREE.PointLight(0xffc678, 1.2, 75, 2.0);
    underLight.position.set(0, 8, 0);
    scene.add(underLight);

    const warmCityLight = new THREE.PointLight(0xffb966, 1.45, 52, 2.0);
  ...
  function animate() {
    ...
    if (now - lastRenderTime < (lowPower ? 32 : 22)) return;
    lastRenderTime = now;
    ...
    if (dynamicObjects.gardenPetals) {
      dynamicObjects.gardenPetals.children.forEach((petal, i) => {
        petal.position.y += Math.sin(t * .8 + i * 1.7) * .0015;
        petal.rotation.z += delta * (.15 + (i % 5) * .04);
        petal.rotation.y += delta * .12;
      });
    }
    if (dynamicObjects.flyingHeart) {
      const heart = dynamicObjects.flyingHeart;
      heart.position.set(7 + Math.sin(t * .16) * 10, 16.2 + Math.sin(t * .57) * 1.8, -24 + Math.cos(t * .12) * 13);
      heart.lookAt(camera.position);
      heart.rotateZ(Math.sin(t * .75) * .16);
      heart.scale.setScalar(2.05 + Math.sin(t * .95) * .09);
    }
    if (roamActive) {
      letterTargets.forEach((marker, index) => {
        marker.position.y = marker.userData.baseY + Math.sin(t * 1.25 + index * 1.6) * .16;
  ...
  window.flowerGardenSetOrbit = (yaw, pitch, zoom) => {
    if (!camera) return;
    const distance = Math.max(17, 52 + zoom * 2);
    const horizontal = distance * Math.cos(.27 + pitch * .5);
    camera.position.set(Math.sin(.59 + yaw) * horizontal, 3.2 + distance * Math.sin(.27 + pitch * .5), Math.cos(.59 + yaw) * horizontal);
  ...
  window.flowerGardenStartExplore = () => {
    ...
    const x = gardenPathX(38);
    roamPosition.set(x, terrainHeight(x,38) + 4.2, 38);
    updateRoamCamera(0, .08);
  };
  ...
  window.flowerGardenMarkLetterFound = index => {
    if (letterTargets[index]) letterTargets[index].visible = false;
  };
  ...
  window.flowerGardenOrbitOverview = (yawDelta, polarDelta, zoomDelta = 0) => {
    ...
    overviewPolar = THREE.MathUtils.clamp(overviewPolar + polarDelta, .35, 1.42);
    overviewDistance = THREE.MathUtils.clamp(overviewDistance * Math.exp(zoomDelta * .0012), 67, 190);
    updateOverviewCamera();
  };
  window.flowerGardenMove = (forward, strafe, yaw, pitch, seconds) => {
    if (!roamActive) return;
    const length = Math.hypot(forward,strafe) || 1;
    const speed = 12 * Math.min(seconds,.15) / length;
    roamPosition.x = THREE.MathUtils.clamp(roamPosition.x + (Math.sin(yaw)*forward + Math.cos(yaw)*strafe)*speed,-53,53);
    roamPosition.z = THREE.MathUtils.clamp(roamPosition.z + (-Math.cos(yaw)*forward + Math.sin(yaw)*strafe)*speed,-53,53);
    const pond = ((roamPosition.x-19)/9)**2 + ((roamPosition.z+13)/7)**2 < 1.2;
    const ground = terrainHeight(roamPosition.x,roamPosition.z) + 4.2;
    const safeHeight = pond ? Math.max(ground,terrainHeight(19,-13)+1.55+3.2) : ground;
    roamPosition.y += (safeHeight-roamPosition.y)*Math.min(1,seconds*6);
    updateRoamCamera(yaw,pitch);
  };
  window.flowerGardenPickLetter = (clientX,clientY) => {
    if (!roamActive || !letterGroup.visible) return null;
    const rect = renderer.domElement.getBoundingClientRect();
    const pointer = new THREE.Vector2((clientX-rect.left)/rect.width*2-1,-(clientY-rect.top)/rect.height*2+1);
    letterRaycaster.setFromCamera(pointer,camera);
    const hit = letterRaycaster.intersectObjects(letterTargets.filter(marker => marker.visible),true)[0];
    if (!hit) return null;
    const index = hit.object.userData.letterIndex;
    return Number.isInteger(index) ? {index,text:hiddenLetters[index].text} : null;
  };
  window.flowerGardenNearbyLetter = () => {
    if (!roamActive) return null;
    const index = hiddenLetters.findIndex((note, i) => letterTargets[i].visible && Math.hypot(note.x-roamPosition.x,note.z-roamPosition.z)<3.2);
    return index < 0 ? null : {index,text:hiddenLetters[index].text};
  };
  window.flowerGardenSetActive = active => {
    renderEnabled = !!active;
    if (renderEnabled && !scene && !initializationFailed) {
      try { init(); }
      catch (error) {
        initializationFailed = true;
        renderEnabled = false;
        console.warn('El jardín 3D no pudo iniciarse; se mantiene la escena alternativa.', error);
      }
    }
  };
